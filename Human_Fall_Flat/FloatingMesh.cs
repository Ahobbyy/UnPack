...
public class FloatingMesh : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			meshVertices[j] = ((Component)this).get_transform().InverseTransformPoint(((Component)hull).get_transform().TransformPoint(meshVertices[j]));
		}
	...
	private void TransformVertices(Matrix4x4 localToWorldMatrix)
	{
		...
		float weight = 0f;
		Vector3 val = Current.Sample(body.get_worldCenterOfMass(), out weight);
		for (int i = 0; i < meshVertices.Length; i++)
		{
			Vector3 val2 = ((Matrix4x4)(ref localToWorldMatrix)).MultiplyPoint3x4(meshVertices[i]);
			Vector3 val3 = Vector3.get_zero();
			if ((Object)(object)body != (Object)null)
			{
				val3 = body.GetPointVelocity(val2);
			}
			Vector3 velocity = Vector3.get_zero();
			float depth = -1f;
			if ((Object)(object)sensor != (Object)null && (Object)(object)sensor.waterBody != (Object)null)
			{
				depth = sensor.waterBody.SampleDepth(val2, out velocity);
				velocity = Vector3.Lerp(velocity, val, weight);
			}
