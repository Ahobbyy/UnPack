...
public class ColliderLabelTriggerVolume : Node, INetBehavior, IReset
{
	...
	private void Update()
	{
		...
		{
			if ((Object)(object)collider == (Object)null || !((Component)collider).get_gameObject().get_activeSelf())
			{
	...
	private bool IsCorrectCollider(Collider other)
	{
		...
		{
			ColliderLabel component = ((Component)other).GetComponent<ColliderLabel>();
			if (Object.op_Implicit((Object)(object)component))
			{
				...
				foreach (string text in array)
				{
					if (component.Label == text)
					{
						if (debugOutput)
						{
							Debug.Log((object)("Correct collider found: " + ((Object)other).get_name()));
						}
						return true;
					}
					if (debugOutput)
					{
						Debug.Log((object)("Other collider label does not match: " + component.Label));
					}
				}
			}
			else if (debugOutput)
			{
				Debug.Log((object)("Other collider does not have a label: " + ((Object)other).get_name()));
			}
		}
		return false;
	}
