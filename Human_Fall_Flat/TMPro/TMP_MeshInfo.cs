	...
	public struct TMP_MeshInfo
	{
		...
		public TMP_MeshInfo(Mesh mesh, int size)
		{
			...
			while (num4 / 4 < size)
			{
				for (int i = 0; i < 4; i++)
				{
					vertices[num4 + i] = Vector3.get_zero();
					uvs0[num4 + i] = Vector2.get_zero();
					uvs2[num4 + i] = Vector2.get_zero();
			...
			this.mesh.set_normals(normals);
			this.mesh.set_tangents(tangents);
			this.mesh.set_triangles(triangles);
			this.mesh.set_bounds(new Bounds(Vector3.get_zero(), new Vector3(3840f, 2160f, 0f)));
		}
		...
		public TMP_MeshInfo(Mesh mesh, int size, bool isVolumetric)
		{
			...
			while (num5 / num < size)
			{
				for (int i = 0; i < num; i++)
				{
					vertices[num5 + i] = Vector3.get_zero();
					uvs0[num5 + i] = Vector2.get_zero();
					uvs2[num5 + i] = Vector2.get_zero();
			...
			this.mesh.set_tangents(tangents);
			this.mesh.set_triangles(triangles);
			this.mesh.set_bounds(new Bounds(Vector3.get_zero(), new Vector3(3840f, 2160f, 64f)));
		}
