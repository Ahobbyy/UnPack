...
namespace Multiplayer
{
	public class NetPlayer : NetScope
	{
		...
		{
			if (App.state != AppSate.PlayLevel && App.state != AppSate.ClientPlayLevel)
			{
				return App.state == AppSate.ServerPlayLevel;
			}
			return true;
		}
		...
		{
			NetPlayer component = ((Component)Object.Instantiate<NetPlayer>(Game.instance.playerPrefab)).GetComponent<NetPlayer>();
			component.human.player = component;
			component.human.ragdoll = Object.Instantiate<GameObject>(((Component)Game.instance.ragdollPrefab).get_gameObject(), ((Component)component.human).get_transform(), false).GetComponent<Ragdoll>();
			component.human.ragdoll.BindBall(((Component)component.human).get_transform());
			...
			{
				((Component)component.cameraController).get_gameObject().SetActive(false);
			}
			if (NetGame.netlog)
			{
				Debug.LogFormat("Spawning {0}", new object[1] { component.netId });
			}
			return component;
		}
		...
		public void DespawnPlayer()
		{
			if (NetGame.netlog)
			{
				Debug.LogFormat("Destroying {0}", new object[1] { netId });
			}
			if (isLocalPlayer)
			{
				if (skin != null)
			...
			if ((Object)(object)((Component)this).get_gameObject() != (Object)null)
			{
		...
		{
			float num = NetFloat.Dequantize(stream.ReadInt32(8), 1f, 8);
			float num2 = NetFloat.Dequantize(stream.ReadInt32(8), 1f, 8);
			lock (moveLock)
			{
				moveFrames++;
				walkForward = Mathf.Lerp(walkForward, num, 1f / (float)moveFrames);
				walkRight = Mathf.Lerp(walkRight, num2, 1f / (float)moveFrames);
				...
				if (shooting)
				{
					Debug.LogError((object)"shooting = true in NetPlayer.cs:357");
				}
				NetStream netStream = NetGame.BeginMessage(NetMsgId.Move);
				try
				{
		...
		{
			NetIdentity netIdentity = ((Component)segment.rigidbody).get_gameObject().AddComponent<NetIdentity>();
			netIdentity.sceneId = (uint)nextIdentityId++;
			NetBody netBody = ((Component)segment.rigidbody).get_gameObject().AddComponent<NetBody>();
			...
			{
				((Component)segment.rigidbody).get_gameObject().SetActive(false);
			}
		}
		...
		public void ApplyPreset(RagdollPresetMetadata preset, bool bake = true)
		{
			...
			{
				customization = ((Component)human.ragdoll).get_gameObject().AddComponent<RagdollCustomization>();
			}
