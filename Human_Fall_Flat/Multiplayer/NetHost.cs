...
namespace Multiplayer
{
	public class NetHost
	{
		...
		private uint trackedUnreliableBytesForGUI;
		...
		public uint GetAndClearUnreliableCount()
		{
			lock (unreliableStreamLock)
			{
				uint result = trackedUnreliableBytesForGUI;
				trackedUnreliableBytesForGUI = 0u;
				return result;
			}
		}
		...
		public void FlushBuffer(NetTransport transport, bool skipLock = false)
		{
			lock (unreliableStreamLock)
			{
				if (unreliableStream.position != 0)
				{
					...
					{
						Profiler.BeginSample("NetHost.Flush");
					}
					...
					trackedUnreliableBytesForGUI += (uint)unreliableStream.UseBuffedSize();
