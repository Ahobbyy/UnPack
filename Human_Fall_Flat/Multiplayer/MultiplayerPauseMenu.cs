...
namespace Multiplayer
{
	public class MultiplayerPauseMenu : MenuTransition
	{
		...
		public override void OnGotFocus()
		{
			...
			if (!Object.op_Implicit((Object)(object)ExitText))
			{
				Debug.LogWarning((object)"EXIT BUTTON reference not set in editor");
				return;
			}
			if (NetGame.isServer)
			{
			...
			((Component)informationBox).get_gameObject().SetActive(true);
			...
			{
				workshopLevelButton.get_gameObject().SetActive(false);
				return;
			}
			workshopLevelButton.get_gameObject().SetActive(true);
