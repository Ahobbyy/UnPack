...
namespace Multiplayer
{
	public class NetFloat
	{
		...
		{
			float num = Dequantize(state0.ReadInt32(fullBits), range, fullBits);
			float num2 = Dequantize(state1.ReadInt32(fullBits), range, fullBits);
			return Mathf.Lerp(num, num2, mix);
		}
		...
		public int CalculateMaxDeltaSizeInBits()
		{
			...
			if (deltaSmall == deltaLarge)
			{
				return num + Mathf.Max((int)fullBits, (int)deltaLarge);
			}
			return num + Mathf.Max((int)fullBits, 1 + deltaLarge);
		}
