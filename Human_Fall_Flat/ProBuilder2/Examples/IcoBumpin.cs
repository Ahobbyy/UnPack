...
namespace ProBuilder2.Examples
{
	...
	public class IcoBumpin : MonoBehaviour
	{
		...
		private Vector3 icoPosition = Vector3.get_zero();
		...
		private void Start()
		{
			...
			Dictionary<int, int> dictionary = pb_IntArrayUtility.ToDictionary(ico.get_sharedIndices());
			for (int j = 0; j < faces.Length; j++)
			{
				outsides[j] = new FaceRef(faces[j], pb_Math.Normal(ico, faces[j]), pb_IntArrayUtility.AllIndicesWithValues(ico.get_sharedIndices(), dictionary, (IList<int>)faces[j].get_distinctIndices()).ToArray());
			}
			original_vertices = (Vector3[])(object)new Vector3[ico.get_vertices().Length];
			Array.Copy(ico.get_vertices(), original_vertices, ico.get_vertices().Length);
			displaced_vertices = ico.get_vertices();
			icoMesh = ico.get_msh();
			icoTransform = ((Component)ico).get_transform();
			faces_length = outsides.Length;
			icoPosition = icoTransform.get_position();
			waveform.set_positionCount(1024);
			if (bounceWaveform)
			{
				((Component)waveform).get_transform().set_parent(icoTransform);
			}
			audioSource.Play();
		}
		...
		private void Update()
		{
			...
			for (int i = 0; i < outsides.Length; i++)
			{
				...
				{
					displaced_vertices[num3] = original_vertices[num3] + val;
				}
			}
			...
			{
				Quaternion localRotation = ((Component)waveform).get_transform().get_localRotation();
				Vector3 eulerAngles = ((Quaternion)(ref localRotation)).get_eulerAngles();
				eulerAngles.x = Mathf.PerlinNoise(Time.get_time() * waveformSpeed, 0f) * 360f;
				eulerAngles.y = Mathf.PerlinNoise(0f, Time.get_time() * waveformSpeed) * 360f;
				((Component)waveform).get_transform().set_localRotation(Quaternion.Euler(eulerAngles));
			}
			icoPosition.y = 0f - verticalBounce + (rms + rms_history) * verticalBounce;
			icoTransform.set_position(icoPosition);
			Array.Copy(fft, fft_history, 4096);
			Array.Copy(data, data_history, 1024);
			rms_history = rms;
			icoMesh.set_vertices(displaced_vertices);
		}
