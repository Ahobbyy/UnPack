...
public sealed class GameProgress : MonoBehaviour
{
	public sealed class ProgressData
	{
		...
		{
			if (store.TryGetValue(hash, out var value) && checkpoint <= value)
			{
				return false;
			}
			store[hash] = checkpoint;
		...
		{
			if (store.TryGetValue(hash, out var value))
			{
				return (int)value;
			}
			return -1;
	...
	{
		ReadProgress();
		GameSave.GetWorkshopCheckpoint(out var workshopLevel, out var checkpoint);
		if (workshopLevel != 0L)
		{
	...
	private void ReadProgressFile(string fileName, ProgressData progressData)
	{
		...
		{
			Debug.LogError((object)("Error loading " + fileName + ": " + ex));
		}
	}
