...
public class SrtSubtitles
{
	...
	public void Load(string name, string srt)
	{
		...
		while (j < array.Length)
		{
			...
			if (j >= array.Length || !int.TryParse(array[j++], out var result) || j >= array.Length)
			{
				continue;
			}
			string[] array2 = array[j++].Split(' ');
			if (array2.Length >= 3 && string.Equals(array2[1], "-->") && TryParseTime(array2[0], out var time) && TryParseTime(array2[2], out var time2) && j < array.Length)
			{
				string text = array[j++];
