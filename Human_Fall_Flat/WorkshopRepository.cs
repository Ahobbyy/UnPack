...
public class WorkshopRepository : MonoBehaviour, IDependency
{
	...
	private UGCQueryHandle_t refreshResults;
	...
	{
		string text = FileTools.ReadAllText(FileTools.Combine("ws:" + levelID, "metadata.json"));
		if (text == null)
		{
			return null;
		}
	...
	private bool CheckValidLevel(ref PublishedFileId_t[] subscriptions, ulong level, bool lobby)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if (array[i].m_PublishedFileId == level)
			{
				return true;
			}
		}
	...
	private IEnumerator RefreshTitleDescription()
	{
		...
		if (levelsCount != 0)
		{
			...
			while (batchCount > 0)
			{
				...
				UGCQueryHandle_t handle = SteamUGC.CreateQueryUGCDetailsRequest(publishFields.ToArray(), (uint)publishFields.Count);
				...
				{
					if (SteamUGC.GetQueryUGCResult(refreshResults, (uint)j, out var pDetails))
					{
						bool flag = false;
						WorkshopItemMetadata workshopItemMetadata = levelsToRefresh[j + startCount];
						if (!workshopItemMetadata.title.Equals(pDetails.m_rgchTitle))
						{
							workshopItemMetadata.title = pDetails.m_rgchTitle;
							flag = true;
						}
						if (!workshopItemMetadata.description.Equals(pDetails.m_rgchDescription))
						{
							workshopItemMetadata.description = pDetails.m_rgchDescription;
							flag = true;
						}
						if (flag)
						{
	...
	private void RefreshSubscriptions()
	{
		...
		foreach (WorkshopLevelMetadata item in levelRepo.BySource(WorkshopItemSource.SubscriptionLobbies))
		{
			levelsToRefresh.Add(item);
		}
		foreach (RagdollPresetMetadata item2 in presetRepo.BySource(WorkshopItemSource.SubscriptionLobbies))
		{
			levelsToRefresh.Add(item2);
		}
		ReloadSubscriptions();
		foreach (WorkshopLevelMetadata item3 in levelRepo.BySource(WorkshopItemSource.Subscription))
		{
			levelsToRefresh.Add(item3);
		}
		((MonoBehaviour)this).StartCoroutine(RefreshTitleDescription());
	}
	...
	public bool ReloadSubscriptions(bool isLobby = false, bool onlyLevels = false)
	{
		...
		try
		{
			...
			{
				if ((SteamUGC.GetItemState(publishedFileId_t) & 4u) != 0)
				{
			...
			for (int j = 0; j < num; j++)
			{
				ReadFolder(isLobby ? WorkshopItemSource.SubscriptionLobbies : WorkshopItemSource.Subscription, "ws:" + array[j].ToString() + "/");
			}
			return result;
		}
		catch (Exception ex)
	...
	{
		return FileTools.LoadBundle(FileTools.Combine(wsMeta.folder, "data"));
	}
	...
	private void LoadBuiltinModels()
	{
		...
		{
			if ((Object)(object)builtinModels[i] != (Object)null)
			{
				WorkshopItemMetadata meta = builtinModels[i].meta;
				modelRepos[meta.itemType].AddItem(WorkshopItemSource.BuiltIn, meta);
			}
		}
		for (int j = 0; j < developmentOnlyModels.Length; j++)
		{
			if ((Object)(object)developmentOnlyModels[j] != (Object)null)
			{
				WorkshopItemMetadata meta2 = developmentOnlyModels[j].meta;
				modelRepos[meta2.itemType].AddItem(WorkshopItemSource.BuiltIn, meta2);
			}
		}
		presetRepo.LoadSkins();
	}
	...
	public void SetLobbyTitle(ulong lobbyID)
	{
		if (lobbyID >= 128)
		{
			foreach (WorkshopLevelMetadata item in instance.levelRepo.BySource(WorkshopItemSource.SubscriptionLobbies))
			{
	...
	public static string GetLobbyTitle(string lobbyTitle)
	{
		...
		{
			lobbyTitle = GetLobbyName((ulong)(lobbyTitle[2] - 48));
		}
		return lobbyTitle;
	}
