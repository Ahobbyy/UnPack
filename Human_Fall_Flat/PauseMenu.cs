...
public class PauseMenu : MenuTransition
{
	...
	{
		((Component)textArea).get_gameObject().SetActive(Game.instance.workshopLevel == null);
		workshopArea.SetActive(Game.instance.workshopLevel != null);
		((Component)reloadLevelButton).get_gameObject().SetActive(Game.instance.workshopLevel != null && Game.instance.workshopLevelIsCustom);
		...
		else
		{
			...
			workshopThumbnail.set_texture((Texture)(object)thumbnailTexture);
			workshopTitle.set_text(Game.instance.workshopLevel.title);
			workshopDescription.set_text(LevelSelectMenu2.RichTextProcess(Game.instance.workshopLevel.description));
	...
	private IEnumerator WaitForJoystickClear()
	{
		...
		while (true)
		{
			...
			{
				if ((Object)(object)item.controls != (Object)null && item.controls.ControllerJumpPressed())
				{
					flag = false;
					break;
				}
	...
	public void ResumeClick()
	{
		...
		{
			((MonoBehaviour)this).StartCoroutine(WaitForJoystickClear());
		}
	}
