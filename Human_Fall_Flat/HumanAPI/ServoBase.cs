	...
	public abstract class ServoBase : Node, INetBehavior
	{
		...
		public override void Process()
		{
			...
			if (SignalManager.skipTransitions)
			{
				...
				{
					position = Mathf.Lerp(minValue, maxValue, signedInput ? ((input.value + 1f) / 2f) : input.value);
				}
				else
				{
		...
		private void FixedUpdate()
		{
			...
			if (mode == ServoMode.Position)
			{
				float num3 = Mathf.Lerp(minValue, maxValue, signedInput ? ((input.value + 1f) / 2f) : input.value);
				speed = Mathf.MoveTowards(speed, Mathf.Sign(num3 - num) * maxSpeed, maxAcceleration * Time.get_fixedDeltaTime());
				position += speed * power.value * Time.get_fixedDeltaTime();
			...
			else
			{
				...
				speed = Mathf.MoveTowards(speed, input.value * maxSpeed, maxAcceleration * Time.get_fixedDeltaTime());
				position += speed * power.value * Time.get_fixedDeltaTime();
				direction = Mathf.Sign(position - num);
				if (direction * num2 < 0f)
				{
			...
			if (num != position)
			{
				velocity.SetValue(Mathf.Clamp01(Mathf.Abs(position - num) / Time.get_fixedDeltaTime() / maxSpeed));
				if (isKinematic)
				{
