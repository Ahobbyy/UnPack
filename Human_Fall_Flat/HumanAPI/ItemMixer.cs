	...
	public class ItemMixer : Node, IReset
	{
		...
		public void Update()
		{
			if (!doneMixing)
			{
				...
				{
					activationTimer += Time.get_deltaTime();
		...
		{
			if (doneMixing || (Object)(object)other == (Object)null)
			{
				return;
			}
			bool flag = false;
			...
			{
				if (!((Component)other).get_gameObject().get_transform().IsChildOf(items[i].item.get_transform()))
				{
					continue;
				}
				flag = true;
				...
				{
					if ((Object)(object)((Component)other).get_gameObject() == (Object)(object)items[i].item)
					{
						GrabManager.Release(((Component)other).get_gameObject());
						SendDestroyIngredient(i, -1);
					}
					else
					{
						for (int j = 0; j < items[i].item.get_transform().get_childCount(); j++)
						{
							Transform child = items[i].item.get_transform().GetChild(j);
							if (((Component)other).get_gameObject().get_transform().IsChildOf(child))
							{
								GrabManager.Release(((Component)other).get_gameObject());
								GrabManager.Release(((Component)child).get_gameObject());
		...
		private void DoDestroyIngredient(int itemIndex, int childIndex)
		{
			...
			GameObject val = ((childIndex >= 0) ? ((Component)item.get_transform().GetChild(childIndex)).get_gameObject() : item);
