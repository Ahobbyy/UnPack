...
public class CustomizationPresetMenu : MenuTransition
{
	...
	public override void OnGotFocus()
	{
		...
		if (items.Count == 0)
		{
			...
			BindButtons();
			EventSystem.get_current().SetSelectedGameObject(((Component)((Component)this).GetComponentInChildren<Selectable>()).get_gameObject());
		}
		else
		{
			list.Bind(items);
	...
	{
		((Component)loadButton).get_gameObject().SetActive(mode == CustomizationPresetMenuMode.Load && selectedItem != null);
		((Component)saveButton).get_gameObject().SetActive(mode == CustomizationPresetMenuMode.Save);
		((Component)deleteButton).get_gameObject().SetActive(!showSubscribed && selectedItem != null);
	}
