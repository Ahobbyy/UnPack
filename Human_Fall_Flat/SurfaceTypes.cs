...
public static class SurfaceTypes
{
	...
	public static SurfaceType Resolve(PhysicMaterial physics)
	{
		...
		try
		{
			string text = ((Object)physics).get_name();
		...
		for (int j = 0; j < num; j++)
		{
			if (((Object)physics).get_name().Contains(array[j]))
			{
		...
		Debug.LogError((object)("No suface for physics material " + ((Object)physics).get_name()));
