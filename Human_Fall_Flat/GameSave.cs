...
public static class GameSave
{
	...
	public const int latestLevelID = 3;
	...
	private static GameSaveState savedState;
	...
	private static void ReadSteam()
	{
		...
		try
		{
			if (SteamRemoteStorage.FileExists("progress.txt"))
			{
				...
				if (@string.Length > 0)
				{
					savedState = JsonUtility.FromJson<GameSaveState>(@string);
				}
			}
		}
		catch (Exception ex)
	...
	private static void WriteSteam()
	{
		if (SteamManager.Initialized)
		{
			try
			{
				string text = JsonUtility.ToJson((object)savedState);
				byte[] array = new byte[Encoding.UTF8.GetByteCount(text)];
				Encoding.UTF8.GetBytes(text, 0, text.Length, array, 0);
				SteamRemoteStorage.FileWrite("progress.txt", array, array.Length);
			}
			catch (Exception ex)
			{
				Debug.LogException(ex);
	...
	{
		if (savedState.progressLevel <= levelNumber)
		{
			if (savedState.progressLevel == levelNumber)
			{
				return savedState.progressCheckpoint >= checkpoint;
			}
			return false;
		}
		return true;
	...
	{
		string path = Path.Combine(Application.get_persistentDataPath(), "characterTexture" + slot + ".png");
	...
	public static Texture2D LoadPlayerTexture(int slot)
	{
		...
		try
		{
			byte[] array = File.ReadAllBytes(Path.Combine(Application.get_persistentDataPath(), "characterTexture" + slot + ".png"));
			if (array != null)
			{
				Texture2D val = new Texture2D(2048, 2048, (TextureFormat)3, true, true);
