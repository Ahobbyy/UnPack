		...
		public void ReceivePacket(byte[] packetData, int bufferLength)
		{
			...
			if ((b & 1) == 0)
			{
				...
				if (!flag && (b & 0x80) == 0)
				{
					...
					lock (receivedPackets)
					{
						ReceivedPacketData obj = receivedPackets.Insert(arg) ?? throw new InvalidOperationException("Failed to insert received packet!");
						obj.time = time;
						obj.packetBytes = (uint)(config.PacketHeaderSize + bufferLength);
					}
				...
				for (int i = 0; i < 32; i++)
				{
					if ((ackBits & (true ? 1u : 0u)) != 0)
					{
						ushort obj2 = (ushort)(ack - i);
						SentPacketData sentPacketData = sentPackets.Find(obj2);
						if (sentPacketData != null && !sentPacketData.acked)
						{
							sentPacketData.acked = true;
							if (config.AckPacketCallback != null)
							{
								config.AckPacketCallback(obj2);
							}
