...
public class ServoSound : MonoBehaviour
{
	...
	private void Awake()
	{
		source = ((Component)this).GetComponent<CrossFadingSource>();
	}

	public void CrossfadeLoop()
	{
		AudioClip clip = (secondMedium ? loopClips2[Random.Range(0, loopClips2.Length)] : loopClips[Random.Range(0, loopClips.Length)]);
	...
	public void Play(float stopIn = 0f)
	{
		isPlaying = true;
		this.stopIn = stopIn;
		AudioClip val = (secondMedium ? startClips2[Random.Range(0, startClips2.Length)] : startClips[Random.Range(0, startClips.Length)]);
		AudioClip clip = (secondMedium ? loopClips2[Random.Range(0, loopClips2.Length)] : loopClips[Random.Range(0, loopClips.Length)]);
		source.Play(val, volume, loop: false);
		source.CrossFade(clip, volume, loop: true, val.get_length() - crossfadeStart, crossfadeStart);
	...
	public void Stop()
	{
		...
		{
			isPlaying = false;
			AudioClip clip = (secondMedium ? endClips2[Random.Range(0, endClips2.Length)] : endClips[Random.Range(0, endClips.Length)]);
