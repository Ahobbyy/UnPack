...
public class HumanHead : WaterSensor, INetBehavior
{
	...
	private void Update()
	{
		...
		for (int i = 0; i < waterBodies.Count; i++)
		{
			...
			flag = true;
			Vector3 velocity;
			float num = waterBody.SampleDepth(((Component)this).get_transform().get_position(), out velocity);
			if (num > 0f)
			{
				diveTime += Time.get_deltaTime();
				if (num > drownDepth)
				{
					diveTime += Time.get_deltaTime() * num;
				}
				if (diveTime > drownTime)
				{
					Game.instance.Drown(((Component)this).GetComponentInParent<Human>());
	...
	private void PlayBubble(float drownPhase)
	{
		...
		humanAudio.underwaterBubble.PlayOneShot(((Component)this).get_transform().get_position(), 1f, 0.9f + 0.2f * drownPhase);
		if (!ReplayRecorder.isPlaying || !NetGame.isClient)
		{
