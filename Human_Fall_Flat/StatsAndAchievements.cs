...
{
	public static bool DebugMultiplier = false;
	...
	private void OnUserStatsReceived(UserStatsReceived_t pCallback)
	{
		...
		if (EResult.k_EResultOK == pCallback.m_eResult)
		{
			...
			for (int i = 0; i < SteamAchievementMap.achievementList.Count; i++)
			{
				...
				if (SteamUserStats.GetAchievement(SteamAchievementMap.AchievementToSteam(achievement), out var pbAchieved) && pbAchieved)
				{
		...
		{
			Debug.Log((object)("RequestStats - failed, " + pCallback.m_eResult));
		}
	}
	...
	private void OnUserStatsStored(UserStatsStored_t pCallback)
	{
		if ((ulong)m_GameID == pCallback.m_nGameID && EResult.k_EResultOK != pCallback.m_eResult)
		{
			...
			{
				Debug.Log((object)("StoreStats - failed, " + pCallback.m_eResult));
			}
		}
	}
	...
	private void OnAchievementStored(UserAchievementStored_t pCallback)
	{
		if ((ulong)m_GameID == pCallback.m_nGameID)
		{
			...
			Debug.Log((object)("Achievement '" + pCallback.m_rgchAchievementName + "' progress callback, (" + pCallback.m_nCurProgress + "," + pCallback.m_nMaxProgress + ")"));
		}
	}
	...
	{
		if (NetGame.isServer && (Object)(object)human != (Object)null && (Object)(object)human.player != (Object)null && !human.player.isLocalPlayer)
		{
	...
	public static void IncreaseJumpCount(Human human)
	{
		...
		else if (IsStatsPlayer(GetLocalPlayerIndex(human)))
		{
			jump += ((!DebugMultiplier) ? 1 : 20);
			if (jump % 100 == 0)
			{
				Save(triggerSave: false);
			}
	...
	public static void AddTravel(Human human, float m)
	{
		...
		{
			travelM += m * (DebugMultiplier ? 100f : 1f);
	...
	public static void AddClimb(Human human, float m)
	{
		...
		if ((NetGame.isServer || NetGame.isLocal) && (Object)(object)human != (Object)null && (Object)(object)human.player != (Object)null && !human.player.isLocalPlayer)
		{
			if (IsStatsPlayer((int)human.player.localCoopIndex))
			{
		...
		{
			climbM += m * (DebugMultiplier ? 10f : 1f);
	...
	public static void AddCarry(Human human, float m)
	{
		...
		{
			carryM += m * (DebugMultiplier ? 20f : 1f);
	...
	public static void AddShip(float m)
	{
		...
		shipM += m * (DebugMultiplier ? 20f : 1f);
	...
	public static void AddDrive(float m)
	{
		...
		driveM += m * (DebugMultiplier ? 20f : 1f);
	...
	public static void AddDumpster(float m)
	{
		...
		dumpsterM += m * (DebugMultiplier ? 5f : 1f);
