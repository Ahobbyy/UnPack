...
public static class WorkshopUpload
{
	...
	private static bool Initialize()
	{
		...
		if (!m_bInitialized)
		{
			Debug.LogError((object)"[Steamworks.NET] SteamAPI_Init() failed. Refer to Valve's documentation or the comment above this line for more information.");
			return false;
		}
		return true;
	}
	...
	{
		if (param.m_eResult == EResult.k_EResultOK && SteamUGC.GetQueryUGCResult(param.m_handle, 0u, out var pDetails) && pDetails.m_eResult == EResult.k_EResultFileNotFound)
		{
			meta.workshopId = 0uL;
			meta.Save(folder);
			Upload();
	...
	{
		Application.OpenURL("steam://url/CommunityFilePage/" + steamId);
	}

	public static void ShowWorkshop()
	{
		Application.OpenURL("steam://url/SteamWorkshopPage/" + 477160);
	}

	public static void ShowLevelWorkshop()
	{
		Application.OpenURL("https://steamcommunity.com/workshop/browse/?appid=477160&requiredtags[]=Levels");
	}

	public static void ShowLobbyWorkshop()
	{
		Application.OpenURL("https://steamcommunity.com/workshop/browse/?appid=477160&requiredtags[]=Lobbies");
	}
