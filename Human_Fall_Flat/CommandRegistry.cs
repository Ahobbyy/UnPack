...
public class CommandRegistry
{
	...
	internal void Execute(string txt)
	{
		current = this;
		currentCommand = txt;
		if (commands.TryGetValue(txt, out var value))
		{
			value();
			return;
		}
		if (commandsStr.TryGetValue(txt, out var value2))
		{
			value2(null);
			return;
		}
	...
	public void OnHelp(string param)
	{
		if (string.IsNullOrEmpty(param))
		{
			foreach (string key in commands.Keys)
			{
				...
				{
					print(helpColor + (helpExpand ? Translate(description[key]) : key) + "</color>");
				}
			}
			foreach (string key2 in commandsStr.Keys)
			{
				...
				{
					print(helpColor + (helpExpand ? Translate(description[key2]) : key2) + "</color>");
				}
			}
		}
		else
