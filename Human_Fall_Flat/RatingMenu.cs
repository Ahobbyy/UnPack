...
public sealed class RatingMenu : MenuTransition
{
	...
	{
		mNeedToShowRatingMenu = false;
		if (Time.get_unscaledTime() - mLevelPlayedTime > 180f && Game.instance.workshopLevel != null && !Game.instance.workshopLevel.folder.StartsWith("lvl:") && Game.instance.workshopLevel.workshopId == mCurrentWorkshopLevel && !mCurrentWorkshopLevelRated)
		{
	...
	{
		Debug.Log((object)("OnSetUserItemVoteResult: " + result.m_eResult));
	}
	...
	public void QueryRatingStatus(ulong item, bool reset = true)
	{
		if (reset)
		{
			...
			mLevelPlayedTime = Time.get_unscaledTime();
		}
