...
namespace I2.Loc
{
	public static class LocalizationManager
	{
		...
		private static List<string> sLanguageList
		{
			get
			{
				if (sLocalisation == null)
				{
					return null;
				}
				return sLocalisation.mColumns;
			}
		}
		...
		public static string GetTermTranslation(string Term, int overrideLanguage = -1)
		{
			...
			int num = ((overrideLanguage != -1) ? overrideLanguage : sLanguageColumn);
			...
			if (value != null)
			{
				...
				{
					Debug.Log((object)("GetTermTranslation() language column missing: " + num));
				}
			}
			else
			{
				Debug.Log((object)("GetTermTranslation() : MISSING KEY!! " + Term));
			}
			if (text2 == null)
			{
				text2 = "Missing: " + Term;
		...
		public static void StartupLanguage()
		{
			...
			if ((Object)(object)hFFMasterLocalisationFile == (Object)null)
			{
				Debug.Log((object)"Accessing the language system too early, localisation asset not loaded yet.");
				return;
			}
			sLocalisation = Load(hFFMasterLocalisationFile.get_text());
		...
		private static void EOL(LocalisationFile localisationFile, StringBuilder builder, int lineNumber)
		{
			...
			{
				Debug.Log((object)("String table already contains: " + localisationFile.mColumns[0]));
			}
			if (localisationFile.mColumns.Count != 22)
			{
				Debug.Log((object)("Incorrect number of columns on line: " + lineNumber));
			}
			builder.Length = 0;
